import { Environment, Logger, MaybeAsyncStorage, StorageContext, StorageManager, StorageService } from "@matter/main";
...
export class ConfigStorage {
    ...
    #storage?: StorageManager<MaybeAsyncStorage>;
    ...
    async open() {
        ...
        this.#configStore = (await this.#storageService.open("controller")).createContext("config");
        ...
        const wifiSsid = (await this.#configStore.has("wifiSsid"))
            ? await this.#configStore.get<string | undefined>("wifiSsid")
            : undefined;
        const wifiCredentials = (await this.#configStore.has("wifiCredentials"))
            ? await this.#configStore.get<string | undefined>("wifiCredentials")
            : undefined;
        const threadDataset = (await this.#configStore.has("threadDataset"))
            ? await this.#configStore.get<string | undefined>("threadDataset")
            : undefined;
